...
local function py2image(code, filetype, dark)
    ...
    local header = "import numpy as np \nimport matplotlib \nmatplotlib.use(\'Agg\') \nimport matplotlib.pyplot as plt\n"
    if (dark) then
      header = header .. "plt.style.use(\'dark_background\')\n"
    end
    extendedCode = header .. extendedCode
...
function CodeBlock(block)
    ...
    if (block.classes[1] == 'py2image') then
      ...
      local success, img = pcall(img_converter, block.text,
        filetype, block.attributes["additionalPackages"] or nil)
      fname = hashed .. "." .. filetype
      pandoc.mediabag.insert(fname, mimetype, img)

      

    elseif success and img then
